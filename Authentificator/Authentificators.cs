...
namespace Authentificator
{
    ...
    public class UserAuthentData
    {
        ...
        public UserAuthentData(string inp_login, string inp_password, string inp_userName, bool inp_isPasswordBlocked, DateTime inp_dateTimeWhenPasswordEnds, string RfidCod)
        {
    ...
    public class Authentificators
    {
        ...
        public Authentificators(string inp_fullPathToUserAuthDataFile, AuthertificatorActions inp_allowedActions, string login, string password)
        {
            ...
            RemoteConnection.Connect(login, password, Path.GetDirectoryName(inp_fullPathToUserAuthDataFile));
            //
        ...
        private void CheckUserName(string inp_userName)
        {
            // Проверки для имени пользователя
            // -------------------------------
            if (String.IsNullOrEmpty(inp_userName) == true)
                throw new Exception("Не задано Имя пользователя");

            var trimUserName = inp_userName.Trim();

            if (trimUserName.Length < 2)
                throw new Exception("Длина параметра Имя пользователя должна быть не менее 2 символов");
            if (trimUserName.Length > 60)
                throw new Exception("Длина параметра Имя пользователя не должна превышать 60 символов");
        }
        ...
        private void CheckLogin(string inp_loginName)
        {
            ...
            this.CheckAuthentDataSpelling(inp_loginName, "Логин");

            if (inp_loginName.Length < 8)
                throw new Exception("Длина параметра Логин должна быть не менее 8 символов");
            if (inp_loginName.Length > 24)
                throw new Exception("Длина параметра Логин не должна превышать 24 символов");
        }
        ...
        public  enum UserAuthentResult:byte
        {
            AuthentDataNotFound,
            OK,
            WrongPassword,
        ...
        /// 
        /// <param name="inp_hashPassword">password</param>
        /// 
        ...
        /// 
        /// <exception cref="System.Exception">any exception while performing method</exception>
        public Tuple<UserAuthentResult, string>  Authenticate(string inp_loginName, byte [] inp_hashPassword)
        {
            // Вначале осуществляем проверку корректности заданных имени пользователя и пароля
            // -------------------------------------------------------------------------------
            ...
            try
            {
                ...
                if (_userAuthentData != null)
                {
                    foreach (UserAuthentData _uad in _userAuthentData)
                    {
                        ...
                        {
                            if (_uad.IsPasswordBlocked == true)
                                return new Tuple<UserAuthentResult, string>(UserAuthentResult.PasswordBlocked, _uad.UserName);

                            if (_uad.Password == _byteConverter.GetString(inp_hashPassword))
                            {// если пароль указан правильно, тогда проверяем остальные параметры для этого пользователя


                                if (_uad.DateTimeWhenPasswordEnds < DateTime.Now)
                                    return new Tuple<UserAuthentResult, string>(UserAuthentResult.PasswordExpired, _uad.UserName);

                                return new Tuple<UserAuthentResult, string>(UserAuthentResult.OK, _uad.UserName);
                            }
                            else
                            {
                                return new Tuple<UserAuthentResult, string>(UserAuthentResult.WrongPassword, _uad.UserName); 
                            }
            ...
            return new Tuple<UserAuthentResult, string>(UserAuthentResult.AuthentDataNotFound, string.Empty);
        } 
                ...
                using (_stream = new FileStream(this.fullPathToUserAuthDataFile, FileMode.Open, FileAccess.Read, FileShare.Read))
                {
                    ...
                    if (_userAuthentData != null)
                    {
                        foreach (UserAuthentData _uad in _userAuthentData)
                        {
                            ...
                            _allUsersInfo.Add(new UserAuthentData(_uad.Login, String.Empty, _uad.UserName,  _uad.IsPasswordBlocked, _uad.DateTimeWhenPasswordEnds, _uad.RfidCod));
        ...
        public void SaveUserAuthentData(UserAuthentData inp_userAuthentData, ViewReader viewReader)
        {
            ...
            this.CheckUserName(inp_userAuthentData.UserName);
            ...
            try
            {
                ...
                _userAuthentData.Add(new UserAuthentData(inp_userAuthentData.Login,
                                                        _byteConverter.GetString(_md5.ComputeHash(_byteConverter.GetBytes(inp_userAuthentData.Password))), inp_userAuthentData.UserName,
        ...
        /// <param name="inp_loginName">the name of the user whose password to edit</param>
        /// 
        /// <param name="inp_userName">new user password</param>
        /// 
        /// <exception cref="System.Exception">any exception while performing method</exception>
        public void EditUserName(string inp_loginName, string inp_userName)
        {
            // Вначале осуществляем проверку корректности заданных имени пользователя и пароля
            // -------------------------------------------------------------------------------
            this.CheckLogin(inp_loginName);
            this.CheckUserName(inp_userName);
            ...
            try
            {
                ...
                _userAuthentData[_userToEditIndex].UserName = inp_userName;
